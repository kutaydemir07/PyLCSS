...
def _keyword_field(value: object) -> str:
    """Format one keyword field without exceeding 10 columns."""
    ...
    if len(text) > 10:
        raise ValueError(
            f"Keyword value does not fit a 10-column field: {value!r}"
        )
    return text.rjust(10)


def _keyword_card(*values: object) -> str:
    """Build one conventional 8-by-10-column keyword card."""

    if len(values) > 8:
        raise ValueError("A keyword card can contain at most 8 fields.")
        ...
        node_ids = (
            nodes_matching_geometries(
                ...
                tolerance=nonnegative_float(
                    ...
                    label="Impact surface search tolerance",
                ),
            )
            + 1
        )
...
def _append_velocity(
    ...
    *,
    ...
    # ``speed * end_time`` is the prescribed travel only for a massless wall.
    # A finite-mass wall decelerates from contact force and can legitimately
    # use a long post-impact window to measure unloading and permanent set.
    # Treating its free-flight distance as commanded travel produced a false
    # overrun warning for exactly the long-duration crush studies that need
    # the extra output time.
    finite_mass_wall = bool(
        moving_rigid_wall
        and not state.prescribed_wall
        and float(impactor_mass) > 0.0
    )
    if not state.moving_body and not finite_mass_wall:
...
def _append_moving_wall(
    ...
    *,
    ...
    wall_slave_set_id = next_set_id
    next_set_id += 1
    wall_candidate_ids = np.arange(1, points.shape[0] + 1, dtype=int)
    if constrained_node_ids:
        # A node must not be both a fixed boundary-condition node and a rigid
        # wall secondary node. OpenRadioss flags that overlap as incompatible
        # kinematics even when the support is geometrically far from the wall.
        # The impact face remains eligible while the fixed fixture is excluded.
        wall_candidate_ids = np.setdiff1d(
            wall_candidate_ids,
            np.asarray(constrained_node_ids, dtype=int),
            assume_unique=False,
        )
    if wall_candidate_ids.size == 0:
        raise SolverBackendError(
            "Every mesh node is constrained, so no nodes remain eligible for "
            "the moving rigid wall. Restrict the support to the fixture region."
        )
    _node_set(
        lines,
        wall_slave_set_id,
