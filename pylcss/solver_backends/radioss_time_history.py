...
def convert_time_history_to_csv(
    ...
    # ``th_to_csv`` is a Windows console executable. Without the headless
    # options it opens its own console window on top of the running study,
    # which is the one place in the impact workflow that still flashed a
    # shell at the user.
    def run_converter(input_path: Path) -> subprocess.CompletedProcess[str]:
        return subprocess.run(
            [str(converter_path), str(input_path)],
    ...
    proc = run_converter(source)
    if proc.returncode != 0 and os.name == "nt":
        # Some official Windows builds of th_to_csv terminate with
        # STATUS_STACK_BUFFER_OVERRUN when the input path/job name is long.
        # Retry from a short temporary basename, then move only the generated
        # CSV back beside the original T-file. The binary input is immutable.
        with tempfile.TemporaryDirectory(prefix="pylcss_th_") as temporary:
            staged = Path(temporary) / "historyT01"
            shutil.copy2(source, staged)
            titles = Path(str(source) + "_TITLES")
            if titles.is_file():
                shutil.copy2(titles, Path(str(staged) + "_TITLES"))
            retry = run_converter(staged)
            staged_csv = Path(str(staged) + ".csv")
            if retry.returncode == 0 and staged_csv.is_file():
                shutil.copy2(staged_csv, expected)
                return expected
            proc = retry
