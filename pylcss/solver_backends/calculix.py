...
from pylcss.solver_backends.calculix_assembly import (
...
def run_calculix_assembly(
    ...
    analysis_type: str = "Linear",
    deformation_scale: Any = "Auto",
    connection_mode: str = "Automatic Bonded",
    connection_tolerance_mm: float = 0.25,
) -> dict:
    """Write and optionally solve a bonded multi-component CalculiX model."""
    warnings: list[str] = []
    allowed_analysis = {"Linear", "Nonlinear (Geometric)", "Nonlinear (Plastic)"}
    if analysis_type not in allowed_analysis:
        raise SolverBackendError(
    ...
    components = record_list(components, label="Components")
    constraints = record_list(constraints, label="Constraints")
    loads = record_list(loads, label="Loads")
    prepared = prepare_calculix_assembly(
        components,
        constraints,
        loads,
        warnings,
        ...
        connection_tolerance_mm=connection_tolerance_mm,
    )
    ...
    inp_path = work_dir / f"{job_name}.inp"
    try:
        inp_path.write_text(prepared.deck, encoding="utf-8")
    except OSError as exc:
        raise SolverBackendError(
            f"Could not write CalculiX input deck {inp_path}: {exc}"
        ) from exc
    ...
    if config.run_solver:
        ...
        solver_log = tail(proc.stdout or "")
        if proc.returncode != 0:
            context = _collect_calculix_failure_context(
                work_dir,
                job_name,
                proc.returncode,
                executable,
        ...
        try:
            ...
            result_payload.update(
                assembly_result_summary(result_payload, prepared)
            )
        except Exception as exc:
            raise SolverBackendError(
                f"CalculiX assembly FRD ingest failed: {exc}"
            ) from exc
    ...
    output = {
        ...
        "message": (
            "CalculiX bonded-assembly deck generated."
            if status == "deck_written"
            else "CalculiX bonded-assembly run complete; results imported."
        ),
    }
    output.update(result_payload)
    return output
