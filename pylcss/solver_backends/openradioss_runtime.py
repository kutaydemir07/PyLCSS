...
# Minimum elements per SMP thread.  Explicit dynamics synchronises every
# thread on every cycle, so a thread that owns too few elements spends more
# time at the barrier than integrating.  Measured on the 3394-shell crashbox
# (0.5 ms, identical deck and I/O, 22 logical CPUs), wall time was:
#
#     1 thread 4.3 s | 2 threads 2.9 s | 4 threads 2.5 s
#     8 threads 3.7 s | 16 threads 7.6 s | 22 threads 24.7 s
#
# Using every logical CPU was 10x slower than the 4-thread optimum, so the
# thread count is derived from model size instead.  800 elements/thread puts
# that model at its measured optimum while still opening up the full machine
# for models large enough to amortise the synchronisation.
_MIN_ELEMENTS_PER_SMP_THREAD = 800


def _engine_thread_count(element_count: int | None) -> int:
    """Choose an SMP width that keeps enough work on each thread.

    ``PYLCSS_OPENRADIOSS_THREADS`` overrides the heuristic for operators who
    have profiled their own models.  ``element_count`` of ``None`` means the
    model size is unknown (a user-supplied deck), in which case
    the full machine is used rather than second-guessing a large model.
    """
    logical_cpus = max(1, os.cpu_count() or 1)
    override = str(os.environ.get("PYLCSS_OPENRADIOSS_THREADS") or "").strip()
    if override:
        try:
            requested = int(override)
        except ValueError:
            logger.warning(
                "Ignoring non-integer PYLCSS_OPENRADIOSS_THREADS=%r.", override
            )
        else:
            if requested >= 1:
                return min(requested, logical_cpus)
            logger.warning(
                "Ignoring PYLCSS_OPENRADIOSS_THREADS=%d; must be >= 1.", requested
            )
    if element_count is None or element_count <= 0:
        return logical_cpus
    affordable = int(element_count) // _MIN_ELEMENTS_PER_SMP_THREAD
    return max(1, min(logical_cpus, affordable))
...
def run_engine(
    ...
    *,
    ...
    """Run Engine with an SMP width matched to the model size."""
    thread_count = _engine_thread_count(element_count)
    logger.info(
        "OpenRadioss Engine: using %d SMP thread(s) for %s element(s) "
        "on %d logical CPU(s)",
        thread_count,
        element_count if element_count else "an unknown number of",
