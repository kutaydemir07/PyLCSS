...
class SeedClusteringMixin:
    def _cluster_seeds_hdbscan(self, seeds_phys: np.ndarray) -> list[np.ndarray]:
        ...
        """
        seeds_phys = np.asarray(seeds_phys, dtype=float)
        if seeds_phys.ndim != 2 or seeds_phys.shape[0] != self.dim:
            raise ValueError(f"seeds_phys must have shape ({self.dim}, n_seeds)")
        if not np.all(np.isfinite(seeds_phys)):
            raise ValueError("seeds_phys must contain only finite values")
        ...
        seeds_norm = (
            seeds_phys - self.active_dsl.reshape(-1, 1)
        ...
        try:
            ...
            logger.warning(
                "hdbscan not available. Treating each seed as individual basin."
            )
