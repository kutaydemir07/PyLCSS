...
class DeflationOptimizationMixin:
    def _find_feasible_regions_via_optimization(
        ...
        found_points_norm: List[np.ndarray] = []
        found_points_phys: List[np.ndarray] = []
        ...
        if solver_type == "nevergrad":
            from ...optimization.solvers.legacy import solve_with_nevergrad as _solve_ng
        else:
        ...
        for i, x_start in enumerate(start_points):
            ...
            try:
                ...
                if violation < 1e-6:
                    ...
                    found_points_phys.append(x_rec * self.active_dv_norm + self.active_dsl)
                    logger.info(
                        f"    Start {i + 1}: new feasible seed (total: {len(found_points_norm)})"
                    )
            except Exception as exc:
                logger.debug(f"    Start {i + 1} failed: {exc}")
                continue
        ...
        if len(found_points_phys) > 0:
            logger.info(
                ...
                f"seed candidates in {i + 1} optimization runs"
            )
