...
"""
Advanced CAD nodes:
    - ImportStepNode: Import STEP/IGES/BREP files into the graph
    - ImportStlNode: Import STL/OBJ/3MF mesh files
...
class ImportStepNode(CadQueryNode):
    """Import a STEP, IGES, or OpenCascade BREP file as geometry."""

    __identifier__ = "com.cad.import_step"
    NODE_NAME = "Import CAD"

    def __init__(self):
        super().__init__()
        self.create_property("filepath", "", widget_type="text")
        self.add_input("file")
        self.add_output("shape")
        legacy_output = self.add_output("shape_out", display_name=False)
        legacy_output.view.setVisible(False)
        self.set_color(100, 160, 100)

    def run(self, **kwargs):
        self.clear_error()
        incoming = resolve_any_input(self.get_input("file"))
...
class ImportStlNode(CadQueryNode):
    """Import an STL, OBJ, or 3MF triangle mesh."""

    __identifier__ = "com.cad.import_stl"
    NODE_NAME = "Import Mesh"

    def __init__(self):
        super().__init__()
        self.create_property("filepath", "", widget_type="text")
        self.add_input("file")
        legacy_input = self.add_input("filepath_in", display_name=False)
        legacy_input.view.setVisible(False)
        self.add_output("mesh")
        legacy_output = self.add_output("mesh_out", display_name=False)
        legacy_output.view.setVisible(False)
        self.set_color(100, 160, 100)
...
class SurfaceAreaNode(CadQueryNode):
    ...
    def __init__(self):
        ...
        legacy_input = self.add_input("shape_in", display_name=False)
        legacy_input.view.setVisible(False)
        self.add_output("area")
        legacy_output = self.add_output("area_out", display_name=False)
        legacy_output.view.setVisible(False)
        self.set_color(120, 180, 120)
    ...
    def run(self, **kwargs):
        ...
        try:
            ...
            if shape is None:
                shape = resolve_shape_input(self.get_input("shape_in"))
            if shape is None:
