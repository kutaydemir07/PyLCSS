...
def _mesh_selection_payload(
    ...
    selection["selected_vertices"] = [
        [float(x), float(y), float(z)] for x, y, z in pts.tolist()
    ]
        ...
        try:
            ...
            if face_arr.ndim == 2 and face_arr.shape[1] >= 3 and face_arr.size:
                ...
                if face_arr.size:
                    ...
                    selection["boundary_kind"] = "surface"
        except Exception:
            pass
    else:
        # Directional selections at the open end of a shell structure are a
        # set of free-edge nodes, not a filled CAD face. Preserve their exact
        # edge connectivity so the viewer can show the supported/impacted
        # perimeter instead of a misleading marker at the centroid.
        try:
            surface = _mesh_surface_triangles(mesh_like)
            if surface is not None and surface.size:
                edge_counts: dict[tuple[int, int], int] = {}
                for a, b, c in np.asarray(surface, dtype=int)[:, :3]:
                    for first, second in ((a, b), (b, c), (c, a)):
                        edge = tuple(sorted((int(first), int(second))))
                        edge_counts[edge] = edge_counts.get(edge, 0) + 1
                selected_ids = {int(value) for value in ids.tolist()}
                boundary_edges = [
                    edge
                    for edge, count in edge_counts.items()
                    if count == 1
                    and edge[0] in selected_ids
                    and edge[1] in selected_ids
                ]
                if boundary_edges:
                    edge_node_ids = sorted(
                        {node_id for edge in boundary_edges for node_id in edge}
                    )
                    local_index = {
                        node_id: index
                        for index, node_id in enumerate(edge_node_ids)
                    }
                    selection["edge_node_ids"] = edge_node_ids
                    selection["edge_vertices"] = [
                        [
                            float(p[0, node_id]),
                            float(p[1, node_id]),
                            float(p[2, node_id]),
                    ...
                    selection["edge_lines"] = [
                        [local_index[first], local_index[second]]
                        for first, second in boundary_edges
                    ]
                    selection["boundary_kind"] = "edge_nodes"
        except Exception:
            pass
...
def _mesh_direction_selection(
    ...
    if _mesh_tetrahedra(mesh_like) is None:
        # Trigger corrugations on an open shell can have normals that point in
        # the requested direction even though they are far from the model end.
        # Prefer exact edge nodes whenever the first qualifying shell face is
        # not actually at the requested coordinate bound.
        node_limit = float(
            np.min(p[axis]) if sign < 0.0 else np.max(p[axis])
        )
        normal_extreme = None
        if np.any(normal_mask):
            normal_centers = centers[normal_mask, axis]
            normal_extreme = float(
                np.min(normal_centers)
                if sign < 0.0
                else np.max(normal_centers)
            )
        if normal_extreme is None or abs(normal_extreme - node_limit) > tol:
            node_tol = _mesh_axis_tolerance(p)
            if sign < 0.0:
                ids = np.where(p[axis] <= node_limit + node_tol)[0]
            else:
                ids = np.where(p[axis] >= node_limit - node_tol)[0]
            return ids.astype(int), None
