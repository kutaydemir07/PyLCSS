...
def _drop_degenerate_faces(
    ...
    *,
    ...
    ``drop_zero_area=False`` keeps exactly-flat triangles. They are worthless on
    their own, but a recovered surface can be closed *because* of them, and a
    closed shell with a few zero-area faces is worth far more downstream than a
    clean triangle soup with a hole in it. See :func:`_mesh_repair_and_validate`.
    """
    vertices = np.asarray(vertices, dtype=float)
    if vertices.ndim != 2 or vertices.shape[1] < 3:
...
def _merge_nearby_vertices(
    ...
    *,
    ...
    welded, remapped_faces = _drop_degenerate_faces(
        welded, remapped_faces, drop_zero_area=drop_zero_area
    )
...
def _mesh_repair_and_validate(
    ...
    *,
    ...
    Zero-area triangles are dropped only when the surface survives it. Snapping
    an extruded result onto its exact end-cap planes leaves a few collinear
    slivers where a swept wall meets a cap, and on a closed shell those slivers
    are load-bearing: deleting them opens boundary edges and this function then
    rejects its own output as non-watertight. OCC sews across a zero-width gap
    within `sew_tolerance` anyway, so keeping them is the cheaper trade.
    """
    dropped = _drop_degenerate_faces(vertices, faces)
    kept = _drop_degenerate_faces(vertices, faces, drop_zero_area=False)
    drop_zero_area = True
    if (
        len(dropped[1]) < len(kept[1])
        and len(dropped[0]) >= 4
        and len(dropped[1]) >= 4
        and not _find_boundary_edges(kept[1])
        and _find_boundary_edges(dropped[1])
    ):
        drop_zero_area = False
        logger.info(
            "Recovered Shape mesh repair: kept %d zero-area triangle(s) that "
            "hold the surface closed.",
            len(kept[1]) - len(dropped[1]),
        )
    vertices, faces = kept if not drop_zero_area else dropped
    if len(vertices) < 4 or len(faces) < 4:
        raise RuntimeError(
    ...
    vertices, faces = _merge_nearby_vertices(
        vertices,
        faces,
        tolerance=float(weld_tolerance) * 0.25,
        drop_zero_area=drop_zero_area,
    )
    ...
    vertices, faces = _drop_degenerate_faces(
        vertices, faces, drop_zero_area=drop_zero_area
    )
