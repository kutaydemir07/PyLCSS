...
"""Density-field to manufacturing-surface recovery."""
...
from .print_surface import (
    ...
    volume_preserving_signed_distance,
)
...
def _recover_voxel_shape(
    ...
    try:
        ...
        use_vtk_sdf = (
            bool(print_ready)
            and str(surface_backend or "").strip().lower()
            not in {"legacy", "marching_cubes", "skimage"}
        )
        if use_vtk_sdf:
            material_mask = iso_field <= float(mc_level)
            try:
                print_sdf, _ = volume_preserving_signed_distance(
                    material_mask,
                    spacing,
                )
                if passive_shapes_present:
