...
The voxel topology optimizer reports a SIMP-relaxed *proxy* stress field
...
CalculiX was chosen deliberately as the project's independent verification
backend; the smoothed STL remains the *manufacturing* deliverable, while this
validates the *engineering* result.
...
# C3D10 midside edges in CalculiX order: nodes 5..10 sit on the
...
def _extract_density_field(
    ...
    cutoff = float(payload.get('density_cutoff', 0.30) or 0.30)
