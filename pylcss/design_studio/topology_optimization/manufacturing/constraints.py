...
"""Manufacturing-constraint density projections applied during optimization."""
...
def _apply_pull_out(x_3d: np.ndarray, pull_axis: str) -> np.ndarray:
    """Remove undercuts so a one-sided mould can be withdrawn along an axis.

    A cast or moulded part must have no material shadowed along the direction
    the tool is pulled: once a column of the design turns to void on the way
    out, it cannot become solid again, or the tool would be trapped. Enforcing
    that is a running minimum of the density along the pull direction.

    This is the constraint commercial tools expose as a *pull-out* or *draw*
    direction, and it is what makes an optimized rib pattern on a housing
    demouldable. It differs from the additive overhang rule, which limits how
    far material may cantilever over the layer below rather than forbidding
    undercuts outright.

    ``pull_axis`` is one of ``{'+x','-x','+y','-y','+z','-z'}`` or ``'none'``,
    naming the direction the tool travels as it is withdrawn.
    """
    if not pull_axis or str(pull_axis).lower() == 'none':
        return x_3d
    key = str(pull_axis).lower()
    if key not in _BUILD_AXIS_MAP:
        return x_3d

    axis, step = _BUILD_AXIS_MAP[key]
    if x_3d.shape[axis] < 2:
        return x_3d

    # Walking the array against the pull direction and taking a running
    # minimum makes each voxel no denser than everything already passed on
    # the way out, which is exactly the no-undercut condition.
    flipper = [slice(None), slice(None), slice(None)]
    if step > 0:
        out = np.minimum.accumulate(x_3d, axis=axis)
    else:
        flipper[axis] = slice(None, None, -1)
        reversed_view = x_3d[tuple(flipper)]
        out = np.minimum.accumulate(reversed_view, axis=axis)[tuple(flipper)]
    return out
