...
class TopologyOptVoxelNode(_TopologyExecutionMixin, CadQueryNode):
    ...
    The result is a solid envelope. A graded cellular result is a different
    design problem — it must not be penalized or projected to 0/1 — and is
    owned by ``LatticeOptVoxelNode``, which subclasses this node and adds the
    cell family and its manufacturing dimensions.
    """

    __identifier__ = "com.cad.sim.topopt_voxel"
    NODE_NAME = "Topology Solver"

    def __init__(self) -> None:
        super().__init__()
        self.add_output("results", color=(180, 255, 180))
        legacy_result = self.add_output(
            "result", display_name=False, color=(180, 255, 180)
        )
        legacy_result.view.setVisible(False)
        ...
        # Both accept any number of connections, so several supports and
        # several forces are wired straight in. That is what made the separate
        # Load Case node redundant: grouping supports and forces into a case
        # node was only ever a way to get more than one of each.
        ...
        self.create_property(
            ...
            "Structural",
            widget_type="combo",
            items=["Structural", "Thermal", "Coupled Structural + Thermal"],
        )
        self.create_property(
            "formulation",
            "Density (SIMP)",
        ...
        # The one length-scale control worth exposing in guided mode: the
        # thinnest member the design may contain, in model units. It replaces
        # reasoning about `rmin` in voxels, and is what a process capability or
        # a drawing note is actually stated in. Zero means program controlled.
        self.create_property(
        ...
        self.create_property(
            ...
            "Program Controlled",
            widget_type="combo",
            items=["Program Controlled", "Manual"],
        )
        self.create_property(
        ...
        self.create_property(
            "validation_displacement_limit_mm",
            0.0,
            widget_type="float",
        )
        self.create_property(
            "validation_yield_safety_factor",
            1.0,
            widget_type="float",
        )
        self.create_property(
        ...
        self.create_property(
            "topology_convergence_levels",
            3,
            widget_type="int",
        )
        # Compatibility property retained for older project files. The output
        # contract is now unambiguous and is decided by whether the result has
        # an exact boundary representation at all: an extrusion-constrained
        # solid envelope and a strut lattice both do and both receive CAD; a
        # minimal-surface or honeycomb lattice does not and stays a mesh.
        self.create_property("generate_cad_after_optimize", True, widget_type="bool")
        self.create_property(
        ...
        self.create_property("density_cutoff", 0.30, widget_type="float")
        self.create_property(
            "visualization",
            "Density",
            widget_type="combo",
        ...
        self.create_property("print_ready_mesh", True, widget_type="bool")
        ...
        self.create_property(
            ...
            "Professional",
            widget_type="combo",
            items=["Standard", "High", "Professional"],
        )
        ...
        # Reduced-order volumetric heat-rejection coefficient in W/(mm^3 K).
        # This is not a wetted-surface or conjugate-fluid convection model.
        # Zero disables it and restores pure conduction.
        self.create_property("convection_coefficient", 0.0, widget_type="float")
        ...
        self.create_property("overhang_angle_deg", 45.0, widget_type="float")
        # Mould withdrawal direction for a cast/moulded part. '+Z' means the
        # tool is pulled in +Z, so no material may be shadowed along that axis
        # and the design stays free of undercuts. This is what makes an
        # optimized rib pattern on a housing demouldable.
        self.create_property(
            "pull_out_axis",
            "None",
            widget_type="combo",
        ...
        self.create_property("maximum_member_size_mm", 0.0, widget_type="float")
    ...
    def _run_embedded_cad_reconstruction(
        ...
        method = str(
            self.get_property("cad_reconstruction_method")
            or "Auto"
        )
        method_key = method.strip().lower()
        if method_key.startswith("smooth"):
            strategy = "spline"
        else:
            # Legacy "Recovered Shape" / "Faceted" values now route to the
            # single validated professional method. There is no mesh fallback.
            strategy = "auto"
        ...
        shape, report = reconstruct_topopt_cad(
            ...
            merge_angle_deg=0.0,
            return_report=True,
            # A strut lattice is reconstructed from these directly and never
            # goes near the isosurface; everything else uses them only to know
            # that per-triangle sewing is the wrong tool for the result.
            structure_options=getattr(self, "_last_structure_options", None),
            member_plan=getattr(self, "_last_member_plan", None),
        )
        self._last_cad_reconstruction_report = report
        return shape
    ...
    def _graph_operating_case(
        ...
        """Convert one TopOpt Load Case node to the typed solver model."""
        if (
            not isinstance(payload, dict)
            or str(payload.get("type") or "").lower() != "topology_operating_case"
        ):
            raise ValueError(
                "The load_cases input accepts TopOpt Load Case nodes only."
            )
        local_bc = VoxelBC()
        ...
        return LoadCase(
            name=str(payload.get("name") or "Load Case"),
