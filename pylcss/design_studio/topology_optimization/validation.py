...
def run_topopt_validation(
    ...
    *,
    ...
    if len(solved) >= 2:
        ...
        if c_last and abs(c_last) > 1e-30:
        ...
        if v_prev and v_last and abs(v_last) > 1e-30:
...
class _TopologyValidationImplementation(CadQueryNode):
    """Validate a topology-optimised result via CalculiX (production FEA).

    Inputs:
        topology_result — connect the Topology Opt (Voxel) node's output; the
                          full result dict (with its `density` field) is read.
        material        — same material dict consumed by SolverNode.
        constraints     — graph constraints (same as SolverNode).
        loads           — graph loads (same as SolverNode).

    Output:
        results — CalculiX FEA result payload, format-compatible with
                  SolverNode so the existing FEA viewer renders it unchanged.
    """
    __identifier__ = 'internal.topopt_validation'
    NODE_NAME = 'Internal Topology Validation'
    ...
    def __init__(self):
        ...
        # Validate against the as-optimised density at this cutoff.  0 means
        # "use the cutoff the optimiser already reported".
        self.create_property('validation_cutoff', 0.0, widget_type='float')

        # Element order for the validation solve.  Quadratic C3D10 tets are far
        # less stiff than linear C3D4 and resolve stress gradients the linear
        # element smears — the default for trustworthy sign-off.
        self.create_property('element_order', 'Quadratic (C3D10)', widget_type='combo',
                             items=['Quadratic (C3D10)', 'Linear (C3D4)'])
        # Mesh-convergence study: solve at this many refinement levels (voxel
        # supersampling factors 1, 2, 3, …) and report whether the result has
        # converged.  1 = single solve (no study).  Cost grows ~factor³.
        self.create_property('convergence_levels', 1, widget_type='int')
        # Safety valve: stop adding finer levels once an estimated element count
        # would exceed this budget (the base level always runs regardless).
        self.create_property('max_validation_elements', 500000, widget_type='int')

        # CalculiX dispatch (mirrors SolverNode).
        ...
        # Analysis options (mirrors SolverNode).
    ...
    def run(self):
        ...
        topo_payload    = self.get_input_value('topology_result', None)
        ...
        if field is None:
            missing.append("a topology-opt result with a density field "
                           "(connect the Topology Opt (Voxel) node)")
        if material is None:
            missing.append("material")
        ...
        if not load_list:
            missing.append("at least one load")
        if missing:
            self.set_error("Topology validation requires "
                           + ", ".join(missing) + ".")
            return None

        density, bounds, reported_cutoff = field
        cutoff = float(self.get_property('validation_cutoff') or 0.0)
        if cutoff <= 0.0:
            cutoff = reported_cutoff

        order_label = str(self.get_property('element_order') or '').upper()
        quadratic = ('C3D10' in order_label) or ('QUADRAT' in order_label)
        elem_label = 'C3D10' if quadratic else 'C3D4'

        try:
            n_levels = max(1, int(self.get_property('convergence_levels') or 1))
        except (TypeError, ValueError):
            n_levels = 1
        try:
            elem_budget = max(1, int(self.get_property('max_validation_elements') or 500000))
        except (TypeError, ValueError):
            elem_budget = 500000

        viz = str(self.get_property('visualization') or 'Von Mises Stress')
        analysis = str(self.get_property('analysis_type') or 'Linear')
        mat = material if isinstance(material, dict) else {}
        run_solver = (
            as_bool(self.get_property('run_external_solver'))
            and not as_bool(self.get_property('deck_only'))
        )
        ...
        def _solve_at(refine: int):
            ...
            logger.info(
                "TopologyOptValidation: level %d (%s) — %d tets / %d nodes",
                refine, elem_label, mesh.t.shape[1], mesh.p.shape[1],
            )
            config = ExternalRunConfig(
                executable=(self.get_property('external_solver_path') or None) or None,
                work_dir=(self.get_property('external_work_dir') or None) or None,
                run_solver=run_solver,
                timeout_s=float(self.get_property('external_timeout_s') or 3600.0),
                job_name=f"pylcss_topopt_validation_L{refine}",
            )
            out = run_calculix_static(
                mesh, mat, constraint_list, load_list, config,
                visualization_mode=viz, analysis_type=analysis,
            )
            return mesh, out

        # The base level (factor 1) always runs; finer levels are added until
        # the projected element count would exceed the budget.  A study needs
        # the solver to actually run, so deck-only mode stays single-level.
        try:
            base_solid = _solid_voxel_count(density, cutoff)
        except Exception:
            base_solid = 0
        ...
        try:
            ...
            # Meshing failure (empty field / bad cutoff) on the base level.
            if finest_output is None:
                self.set_error(str(exc))
                return None
        except SolverBackendError as exc:
            if finest_output is None:
                self.set_error(f"CalculiX validation failed: {exc}")
                return None
            # A finer level failed but a coarser solve already succeeded — keep
            # it rather than discarding the whole validation.
            levels.append({'refine': None, 'error': str(exc)})

        if finest_output is None:
            self.set_error("Topology validation produced no CalculiX result.")
            return None
        output = finest_output

        # ── Convergence verdict ────────────────────────────────────────────
        # Base the verdict on compliance (a global, mesh-convergent quantity);
        # report the peak-von-Mises trend too but warn that local peaks at point
        # loads / supports / sharp corners are singular and need not converge.
        tol = 0.05
        solved = [L for L in levels if L.get('solved') and L.get('compliance')]
        comp_rel = vm_rel = None
        converged: Optional[bool] = None
        ...
        if converged is None:
            note = ("Single mesh level — convergence not assessed. Set "
                    "convergence_levels >= 2 for a mesh-convergence check.")
        elif converged:
            note = (f"Compliance changed {comp_rel*100:.1f}% between the two finest "
                    f"meshes (<= {tol*100:.0f}% tol): global response converged.")
            if vm_rel is not None and vm_rel > tol:
                note += (f" Peak von Mises still moved {vm_rel*100:.1f}% — usually a "
                         "stress singularity at a point load, support, or sharp "
                         "re-entrant corner, not a real divergent stress; interpret "
                         "the peak with care.")
        else:
            note = (f"Compliance changed {comp_rel*100:.1f}% between the two finest "
                    f"meshes (> {tol*100:.0f}% tol): NOT converged — raise "
                    "convergence_levels before trusting the result.")
        if budget_capped:
            note += (" Refinement was capped by max_validation_elements; raise it "
                     "for a finer study.")
        ...
        if isinstance(warns, list):
            if converged is False:
                warns.append("Validation mesh NOT converged: " + note)
            elif converged and vm_rel is not None and vm_rel > tol:
                warns.append("Validation peak stress may be singular: " + note)

        # Surface a few extras for the viewer / downstream comparison.  Keep
        # the numeric auto-scale computed by the CalculiX importer unless the
        # user explicitly picked a fixed display multiplier such as "10x".
        scale_prop = str(self.get_property('deformation_scale') or 'Auto').strip()
        if scale_prop.lower() != 'auto':
            try:
                output['deformation_scale'] = float(scale_prop.lower().rstrip('x'))
            except Exception:
        ...
        output['source'] = 'topology_opt_validation'
        output['validation_cutoff'] = cutoff
        # If the optimiser reported a proxy stress, pass it through so a
        # downstream node/UI can compare proxy-vs-CalculiX directly.
        if isinstance(topo_payload, dict) and topo_payload.get('stress_pnorm') is not None:
            output['topopt_proxy_stress_pnorm'] = float(topo_payload['stress_pnorm'])
        return output
