...
CRASH_MATERIAL_PRESETS = {
    ...
    'DP780 Saturating Curve (Concept)': {
        'E': 210000.0, 'nu': 0.28, 'rho': 7.83e-9,
        'yield_strength': 560.0, 'tangent_modulus': 1800.0, 'failure_strain': 0.22,
        'strain_rate_c': 0.0, 'strain_rate_p': 0.0,
        # Concept-screening true-flow curve. It avoids extrapolating the
        # initial bilinear tangent through the very large local strains of a
        # folded shell. Replace it with lot/thickness/rate coupon data before
        # using the result for a safety decision.
        'plastic_strain_curve': [0.0, 0.02, 0.05, 0.10, 0.15, 0.22, 0.50, 25.0],
        'flow_stress_curve_mpa': [560.0, 650.0, 720.0, 800.0, 860.0, 920.0, 970.0, 1000.0],
    },
...
# ─────────────────────────────────────────────────────────────────────────────
# Impact material card
# ─────────────────────────────────────────────────────────────────────────────
# The impact material is part of the shared Material node rather than a node of
# its own: a part has one material, and splitting it in two meant the static and
# the impact study could silently disagree about the same steel. The property
# set and the payload below are unchanged; only where they are edited moved.

IMPACT_MATERIAL_PRESETS = tuple(CRASH_MATERIAL_PRESETS.keys())


def add_impact_material_properties(node) -> None:
    """Attach the plasticity / rate / traceability fields to a Material node."""
    node.create_property('failure_strain', 0.20, widget_type='float')
    # Element deletion is unsafe without a calibrated failure model, so it
    # is deliberately opt-in.
    node.create_property('enable_fracture', False, widget_type='checkbox')
    # Engineering-facing switch only. Cowper-Symonds constants are selected
    # internally from the material preset.
    node.create_property('strain_rate_sensitive', True, widget_type='checkbox')
    # Traceability is deliberately separate from the material preset. Preset
    # values are engineering starting points, not a certification that the
    # exact sheet lot/thickness/rate range has been characterised.
    node.create_property(
        'validation_status', 'Unvalidated',
        widget_type='combo',
        items=['Unvalidated', 'Validated for this lot and rate range'],
    )
    node.create_property('material_lot_id', '', widget_type='text')
    node.create_property('validation_report_path', '', widget_type='text')
    node.create_property('validated_rate_min_per_s', 0.0, widget_type='float')
    node.create_property('validated_rate_max_per_s', 0.0, widget_type='float')


def impact_material_payload(node, *, elastic=None):
    """Build the impact material card from *node*'s properties.

    ``elastic`` carries the already-resolved (E, nu, rho) from the shared
    Material node. A preset that exists in the impact catalogue brings its own
    calibrated elastic constants and flow curve; any other preset keeps the
    elastic values the static study is using and takes plasticity from the
    node's explicit fields, so the two studies cannot describe different
    materials.
    """
    node.clear_error()
    preset = node.get_property('preset')
    if preset != 'Custom' and preset in CRASH_MATERIAL_PRESETS:
        ...
        H   = p['tangent_modulus']
        ef  = p['failure_strain']
        if _as_bool(node.get_property('strain_rate_sensitive')):
            src = float(p.get('strain_rate_c', 0.0) or 0.0)
            srp = float(p.get('strain_rate_p', 0.0) or 0.0)
        else:
            src = 0.0
            srp = 0.0
        plastic_strain_curve = p.get('plastic_strain_curve')
        flow_stress_curve_mpa = p.get('flow_stress_curve_mpa')
    else:
        try:
            # Elastic constants come from whatever the shared Material node
            # resolved -- its own preset or its custom fields -- so an impact
            # study and a static study on the same node cannot disagree about
            # E, nu or rho. Only plasticity is read from the impact fields.
            if elastic is not None:
                E, nu, rho = (float(value) for value in elastic)
            else:
                E   = float(node.get_property('youngs_modulus'))
                nu  = float(node.get_property('poissons_ratio'))
                rho = float(node.get_property('density'))
            sy  = float(node.get_property('yield_strength'))
            H   = float(node.get_property('tangent_modulus'))
            ef  = float(node.get_property('failure_strain'))
        except (TypeError, ValueError):
            node.set_error("Impact material properties must be numeric.")
            return None
        if _as_bool(node.get_property('strain_rate_sensitive')):
            ...
            src = 0.0
            srp = 0.0
        plastic_strain_curve = None
        flow_stress_curve_mpa = None

    # No yield stress means no plastic material card, which is the ordinary
    # state of a Material node feeding a linear static study. Return nothing
    # rather than erroring: the impact output is simply not offered, and the
    # impact solver already reports a missing material card itself. Erroring
    # here would make every purely elastic material node red.
    if sy <= 0.0:
        return None

    values = (E, nu, rho, sy, H, ef, src, srp)
    if not all(math.isfinite(float(value)) for value in values):
        node.set_error("Impact material properties must be finite numbers.")
        return None
    if E <= 0.0:
        node.set_error("Young's modulus must be greater than zero.")
        return None
    if not (-1.0 < nu < 0.5):
        node.set_error("Poisson's ratio must be between -1 and 0.5 (exclusive).")
        return None
    if rho <= 0.0:
        node.set_error("Density must be greater than zero.")
        return None
    if H < 0.0 or H >= E:
        node.set_error("Tangent modulus must be non-negative and smaller than Young's modulus.")
        return None
    if _as_bool(node.get_property('enable_fracture')) and ef <= 0.0:
        node.set_error("Failure strain must be greater than zero when fracture is enabled.")
        return None
    if src < 0.0 or srp < 0.0:
        node.set_error("Strain-rate constants cannot be negative.")
        return None

    validation_label = str(node.get_property('validation_status') or 'Unvalidated')
    lot_id = str(node.get_property('material_lot_id') or '').strip()
    report_value = str(node.get_property('validation_report_path') or '').strip()
    ...
    try:
        rate_min = float(node.get_property('validated_rate_min_per_s') or 0.0)
        rate_max = float(node.get_property('validated_rate_max_per_s') or 0.0)
    except (TypeError, ValueError):
    ...
    if validation_inputs_complete:
        validation = validate_material_dossier(
            ...
            failure_model_required=_as_bool(
                node.get_property('enable_fracture')
            ),
        )
        ...
        validation = {
            ...
            'validated_rate_range_per_s': [rate_min, rate_max],
            'reason': (
                'The material card is suitable for concept screening only. '
                'No traceable coupon, lot, thickness, and strain-rate dossier '
                'was declared.'
            ),
        }
    ...
    result = {
        ...
        'tangent_modulus': H,
        'failure_strain': ef,
        'enable_fracture': _as_bool(node.get_property('enable_fracture')),
    ...
    if plastic_strain_curve is not None:
        result['plastic_strain_curve'] = list(plastic_strain_curve)
        result['flow_stress_curve_mpa'] = list(flow_stress_curve_mpa)
    return result
