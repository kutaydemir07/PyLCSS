...
def _finite_max_abs(values: object) -> float | None:
    arr = _array(values)
...
def evaluate_crash_quality(
    ...
    material_valid = material_validation.get("status") == "pass"
    checks.append(
        _check(
            "material_validation",
            "pass" if material_valid else "fail",
            material_validation.get("status", "missing"),
    ...
    for check_id, values, label in (
        ...
        checks.append(
            _check(
                ...
                int(finite.size),
                f"{sample_count} finite, non-zero samples",
                f"A qualified crash result requires a physical {label} channel.",
    ...
    independent_kinematics = (
        processing.get("independent_wall_kinematics") is True
    )
    checks.append(
        _check(
            ...
            (
                "Work-energy and impulse-momentum closure must use the "
                "solver's rigid-wall main-node motion, not motion reconstructed "
                "from the same force history."
            ),
    ...
    checks.append(
        _check(
            ...
            "Altair Radioss computation checks",
        )
    )
    ...
    checks.append(
        _check(
            ...
            "Altair Radioss nodal time-step/results-checking guidance",
        )
    )
    ...
    if impactor_mass > 0.0 and max(abs(impulse), impactor_mass * abs(delta_v)) > 1.0e-9:
    ...
    checks.append(
        _check(
            ...
            momentum_error_pct,
            "<= 5% pass; <= 15% warning",
            "Rigid-wall impulse is compared with impactor momentum change.",
    ...
    physical_validation_status = next(
        str(check["status"])
        for check in checks
        if check["id"] == "material_validation"
    )
