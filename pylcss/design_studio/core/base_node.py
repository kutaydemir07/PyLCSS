...
class CadQueryNode(BaseNode):
    ...
    def __init__(self, *args: object, **kwargs: object) -> None:
        ...
        # ``pending`` is deliberately separate from ``error``.  Interactive
        # source nodes can be valid graph members before they have produced a
        # value (for example, a new FreeCAD Part that the user still needs to
        # author).  The executor uses this state to pause that branch without
        # presenting an expected editing step as a computation failure.
        self._pending_message: str | None = None
        # Universal engineering metadata. These fields are intentionally
        # non-execution properties: documenting an assumption must never
        # invalidate geometry or launch a solver.
        self.create_property("description", "", widget_type="text")
        self.create_property("tags", "", widget_type="text")
        self.create_property("notes", "", widget_type="text")
    ...
    def set_pending(self, message: object) -> None:
        """Mark an incomplete-but-valid editing state.

        Pending nodes do not fail graph execution.  Their downstream branch
        waits until the user supplies the missing external or interactive
        result, while the inspector can still explain the next action.
        """
        self.clear_error()
        self._pending_message = str(message)

    def clear_pending(self) -> None:
        """Clear an incomplete editing state after a successful run."""
        self._pending_message = None

    def get_pending(self) -> str | None:
        """Return the current pending-state explanation."""
        return self._pending_message

    def has_pending(self) -> bool:
        """Return whether this node is waiting for an editing step."""
        return bool(self._pending_message)
