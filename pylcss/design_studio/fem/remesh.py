...
class RemeshNode(CadQueryNode):
    ...
    def run(self):
        ...
        if recovered_shape is None:
            self.set_warning("Connect a TopOpt result or recovered_shape surface.")
            return None
        ...
        try:
            ...
            self.set_warning(
                "Remesh Surface could not create a reliable volume mesh. "
                "Use TopOpt validation for engineering checks, or export the "
                "recovered surface as STL/faceted STEP."
            )
            logger.warning(
                "RemeshNode: reliable remesh failed; skipped experimental "
                "Delaunay fallback because it can produce broken TopOpt meshes."
            )
