...
class CustomBlockNode(BaseNode):
    ...
    def __init__(self) -> None:
        ...
        # NodeGraphQt button widgets do not create serializable model
        # properties, although BaseNode.update_model expects them to exist.
...
class SimulationFunctionNode(CustomBlockNode):
    """Managed function block linked to a saved Design Studio study."""
    ...
    def __init__(self) -> None:
        ...
        self.add_text_input(
            "study_summary",
            "Study",
            text="Design Studio - Not configured",
            tooltip="Saved Design Studio project and selected analysis",
        )
        summary_widget = self.get_widget("study_summary")
        if summary_widget:
            summary_widget.get_custom_widget().setReadOnly(True)
        manage_widget = self.get_widget("manage_ports")
        if manage_widget:
            manage_widget.setVisible(False)

    def configure_from_spec(self, spec) -> None:
        """Install a validated bridge interface and its generated adapter."""
        ...
        self.set_property(
            "code_content",
            generate_simulation_function_code(spec),
            push_undo=False,
        )
        ...
        self.set_property(
            "study_summary",
            f"{Path(spec.project_path).stem} - {spec.analysis_kind}",
            push_undo=False,
        )
        self._refresh_function_summary()
