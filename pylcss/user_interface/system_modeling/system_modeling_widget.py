...
class ModelingWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent: QtWidgets.QWidget | None = None) -> None:
        ...
        self._theme_action_icons = {
            ...
            self.action_import_study: ("fa5s.cube", "primary"),
        }
    ...
    def import_design_studio_study(self, project_path=None):
        """Select a saved study and create a managed simulation function."""
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(self, "Cannot Import Study", str(exc))
            return None
        if not descriptor.analyses:
            QtWidgets.QMessageBox.warning(
                self,
                "No Analysis Found",
                "This Design Studio file has no runnable FEA, crash, or "
    ...
    def create_design_studio_function(
        ...
        *,
        ...
        if create_io_nodes:
            input_origin = -75.0 * max(0, len(spec.inputs) - 1)
            for index, item in enumerate(spec.inputs):
                variable = self.create_node_for_graph(graph, InputNode)
                if variable is None:
                    raise RuntimeError(
                        f"Could not create design variable {item.port_name!r}."
                    )
            ...
            for index, item in enumerate(spec.outputs):
                ...
                result.set_pos(430.0, output_origin + index * 170.0)
        ...
        try:
            ...
            logger.debug("Could not fit imported study nodes.", exc_info=True)
