...
from pylcss.user_interface.common.theme_manager import COLORS, current_theme
...
class CodeEditorDialog(QtWidgets.QDialog):
    ...
    def _build_coupling_tab(self) -> QtWidgets.QWidget:
        ...
        self.solver_combo.addItem(
            "Crash — impact response and absorbed energy", "crash"
        )
        self.solver_combo.addItem(
            "Topology optimization — optimized material layout", "topopt"
        )
        ...
        try:
            ...
            if solvers:
                details.append(
                    ...
                    + ", ".join(
                        solver_names[k]
                        for k in ("fea", "crash", "topopt")
                        if k in solvers
                    )
                )
                detected = next(
                    (k for k in ("fea", "crash", "topopt") if k in solvers), None
                )
    ...
    def _refresh_var_list(self) -> None:
        ...
        if hasattr(self, "input_list"):
            ...
            input_color = "#0969da" if current_theme() == "light" else "#38c8e8"
            for name in inputs:
        ...
        if hasattr(self, "output_list"):
            ...
            output_color = "#1a7f37" if current_theme() == "light" else "#36c98f"
            for name in outputs:
