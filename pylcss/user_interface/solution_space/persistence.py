            ...
            if "Platform" in results and results["Platform"] is not None:
                ...
                if i < platform.shape[0]:
                    ...
                    if p_min < p_max:  # Valid intersection
                        ...
                        text.setPos(p_max, 0)
                        text.setFont(QtGui.QFont("Segoe UI", 10, QtGui.QFont.Bold))
                        p.addItem(text)
    ...
    def save_to_folder(self, folder_path):
        ...
        if self.problem:
            problem_data = {
                ...
                # Large numerical arrays live only in the HDF5 companion.
                # Keeping a second list-expanded copy in JSON made project
                # saves consume large amounts of memory and appear frozen.
                "samples": {},
                "results": {},
        ...
        try:
            with h5py.File(temporary_h5, "w") as h5f:
                ...
                if self.last_samples:
                    for key, value in self.last_samples.items():
                        if value is not None:
                            h5f.create_dataset(f"last_samples/{key}", data=value)
                            QtWidgets.QApplication.processEvents(
                                QtCore.QEventLoop.ExcludeUserInputEvents
                            )
                h5f.flush()
    ...
    def load_from_folder(self, folder_path):
        ...
        try:
            ...
            if p_data:
                ...
                if os.path.exists(h5_path):
                    with h5py.File(h5_path, "r") as h5f:
                        ...
                        if "last_samples" in h5f:
                            self.last_samples = {
                                k: np.array(v)
                                for k, v in h5f["last_samples"].items()
                            }
                else:
            ...
            if self.problem and self.system_code:
                ...
                # Keep the already compiled problem. Re-selecting the system
                # here used to execute the generated source a second time on
                # the GUI thread and then immediately discard that duplicate.
    ...
    def save_project(self):
        ...
        if path:
            ...
            try:
                ...
                if self.problem or self.last_samples:
                ...
                if os.path.exists(h5_path):
                    try:
                        with h5py.File(h5_path, "r") as h5f:
                            ...
                            if "last_samples" in h5f:
                                self.last_samples = {
                                    key: h5f[f"last_samples/{key}"][:]
                                    for key in h5f["last_samples"].keys()
                                }
                    except Exception:
