...
class AllDimensionsGraphMixin:
    def compute_adg(self):
        ...
        try:
            ...
            num_inputs = len(
                [n for n in G.nodes() if G.nodes[n]["node_type"] == "input"]
            )
            num_outputs = len(
                [n for n in G.nodes() if G.nodes[n]["node_type"] == "output"]
            )
            num_intermediate = len(
                [n for n in G.nodes() if G.nodes[n]["node_type"] == "intermediate"]
            )
            num_blackbox = len(
                [n for n in G.nodes() if G.nodes[n]["node_type"] == "blackbox"]
            )
            scope_label = selected_scope or "Merged System"
            self.lbl_adg_info.setText(
                f"Scope: {scope_label} | {G.number_of_nodes()} nodes ({num_inputs} inputs, {num_outputs} outputs, {num_intermediate} intermediate, {num_blackbox} functions), {G.number_of_edges()} connections"
            )
