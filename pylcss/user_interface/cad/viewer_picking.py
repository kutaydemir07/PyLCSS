...
class PickingMixin:
    ...
    def _exit_other_picking_modes(self, keep):
        """Leave every picking mode except *keep* before entering it.

        The three modes each set their own flag and none cleared the others, so
        starting an edge session while a face session was open left both flags
        true. Dispatch precedence hid that until the edge session ended: face
        mode survived it, rotation stayed locked, and the next click went to
        the face picker. The symptom was a viewer that "still looks for faces"
        after you asked for edges.
        """
        if keep != "face" and self._picking_mode:
            self.disable_picking_mode()
        if keep != "edge" and self._edge_picking_mode:
            self.disable_edge_picking_mode()
        if keep != "vertex" and self._vertex_picking_mode:
    ...
    def enable_picking_mode(self, multi_select=True):
        ...
        self._exit_other_picking_modes("face")
        ...
        # Preselection: highlight what a click would take, the way CAD
        # packages do. Only runs while a picking mode is active.
        self._enable_hover_preselect("face")
    ...
    def enable_edge_picking_mode(self, multi_select=True):
        ...
        Returns False when the displayed geometry carries no CAD edges. Without
        that check the mode still switched on: the toolbar announced "Edge
        Picking Mode", rotation locked, and every click hit nothing, because a
        mesh result has no B-rep edge actor to pick. Vertex picking already
        refused this way; edge picking silently pretended to work.
        """
        if self._edge_actor is None or not self._all_occ_edges:
            return False
        self._exit_other_picking_modes("edge")
        ...
        self._enable_hover_preselect("edge")
        self.vtkWidget.setCursor(QtGui.QCursor(QtCore.Qt.CrossCursor))
        return True
    ...
    def enable_vertex_picking_mode(self, multi_select=True):
        ...
        if self._vertex_actor is None or not self._all_occ_vertices:
            return False
        self._exit_other_picking_modes("vertex")
