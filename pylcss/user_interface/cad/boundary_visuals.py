...
def dof_caption(fixed_dofs: Iterable[int] | None) -> str:
    ...
    return " ".join(names[index] for index in indices) or "FREE"
...
def compact_constraint_label(metadata: dict | None = None) -> str:
    ...
    if "thermal sink" in normalized:
        return "SINK"
    return constraint_type.upper()[:16]


def legend_caption(metadata: dict | None = None) -> str:
    """Return the legend caption naming a constraint's engineering category."""
    metadata = metadata or {}
    override = str(metadata.get("legend_label") or "").strip()
    if override:
        return override.upper()
    text = " ".join(
        str(metadata.get(key) or "")
        for key in ("visual_kind", "constraint_type", "region_type")
    ).lower()
    if "joint" in text:
        return "JOINT"
    if "keep void" in text or "non-design void" in text:
    ...
    if "displacement" in text or "prescribed" in text:
        return "PRESCRIBED U"
    return compact_constraint_label(metadata) or "CONSTRAINT"


def merge_magnitude_range(minimum: str, maximum: str) -> str:
    """Join two formatted magnitudes, printing a shared unit only once."""
    low, high = str(minimum).strip(), str(maximum).strip()
    if not low or not high:
        return low or high
    low_parts, high_parts = low.rsplit(" ", 1), high.rsplit(" ", 1)
    if len(low_parts) == 2 and len(high_parts) == 2 and low_parts[1] == high_parts[1]:
        return f"{low_parts[0]}–{high_parts[0]} {high_parts[1]}"
    return f"{low}–{high}"
