...
class GenericInspectorMixin:
    # Trailing tokens in a property name that encode the physical unit rather
    # than a word.  `minimum_member_size_mm` has to read "Minimum Member Size
    # (mm)", not "Minimum Member Size Mm".  Longest suffix wins, so `per_s`
    # is matched before the single-token `s`.
    _PROPERTY_UNIT_SUFFIXES = (
        (("per", "s"), "1/s"),
        (("mm", "3"), "mm^3"),
        (("mm", "2"), "mm^2"),
        (("mm",), "mm"),
    ...
    def _human_property_label(cls, property_name):
        ...
        if explicit:
            return explicit
        tokens = [t for t in str(property_name or "").strip("_").split("_") if t]
        unit = ""
        for suffix, symbol in cls._PROPERTY_UNIT_SUFFIXES:
            n = len(suffix)
            # Never strip the whole name down to nothing — `s` alone is a word.
            if len(tokens) > n and tuple(
                t.lower() for t in tokens[-n:]
            ) == suffix:
                tokens = tokens[:-n]
                unit = f" ({symbol})"
                break
        words = [
            cls._PROPERTY_ACRONYMS.get(token.lower(), token.title())
            for token in tokens
        ]
        return (" ".join(words) + unit) if words else "Property"

    @classmethod
    def _section_for(cls, prop_name, node_class=""):
        override = cls._NODE_PROPERTY_SECTIONS.get(str(node_class))
        if override:
            return override
    ...
    def _build_generic_ui(self, node):
        ...
        def _should_skip(name, val):
            ...
            if node.__class__.__name__ == "MeshNode":
                ...
                if name in {"shell_thickness", "shell_nip"} and mesh_type != "Shell":
                    return True
                if name == "refinement_size":
                    refinement_port = node.get_input("refinement_faces")
                    if (
                        refinement_port is None
                        or not refinement_port.connected_ports()
                    ):
                        return True
            ...
            if node.__class__.__name__ == "LoadNode":
                ...
                if name.startswith("gravity_") and not is_gravity:
                    return True
            if node.__class__.__name__ == "MaterialNode":
                if name == "failure_strain" and not bool(props.get("enable_fracture")):
                    return True
            if node.__class__.__name__ in {"NumberNode", "VariableNode"} and name == "value_input":
                return True
            if node.__class__.__name__ == "MathExpressionNode" and name == "expression":
                return True
            if node.__class__.__name__ == "ImpactConditionNode":
                scenario = str(
                    props.get("application_scope")
                    or "Fixed specimen + moving impactor"
                ).strip().lower().replace("_", " ")
                if name in {"impactor_mass_kg", "node_tolerance"} and scenario.startswith(
                    "moving body"
                ):
                    return True
                if name == "impactor_mass_kg" and scenario.startswith(
                    "prescribed moving wall"
                ):
                    return True
            if node.__class__.__name__ == "CrashSolverNode":
                if name == "time_steps" and not bool(
                    props.get("enable_mass_scaling")
                ):
                    return True
                if name == "time_step_scale":
                    return True
            if node.__class__.__name__ == "SolverNode" and name in {
                "assembly_connection",
                "connection_tolerance_mm",
            }:
                # These controls affect only the multi-component CalculiX path.
                # Keeping them visible for the ordinary mesh + material path
                # made a solver implementation detail look like a universal
                # analysis setting.
                try:
                    component_port = node.get_input("components")
                    component_count = len(component_port.connected_ports())
                except (AttributeError, TypeError):
                    component_count = 0
                if component_count < 2:
                    return True
                if (
                    name == "connection_tolerance_mm"
                    and str(props.get("assembly_connection") or "Automatic Bonded")
                    != "Automatic Bonded"
                ):
                    return True
            # Impactor mass moved from the solver into Impact Setup. Keep
            # the serialized solver property for legacy studies without
            # presenting two controls for the same physical value.
            if (
                node.__class__.__name__ == "CrashSolverNode"
                and name == "impactor_mass_kg"
            ):
                return True
            return False

        # Bucket properties into sections, preserving the section order above.
        section_order = [t for t, _ in self._PROPERTY_SECTIONS]
        node_section = self._NODE_PROPERTY_SECTIONS.get(node.__class__.__name__)
        if node_section and node_section not in section_order:
            section_order.insert(0, node_section)
        ...
        for name in sorted(props.keys()):
            ...
            if _should_skip(name, val):
                continue
            buckets[
                self._section_for(name, node.__class__.__name__)
            ].append((name, val))
        ...
        for title in section_order:
            ...
            for name, val in entries:
                ...
                if combo_items:
                    ...
                    if name == "application_scope":
                        display_map = {
                            "Fixed specimen + moving impactor": "Fixed Part + Impactor",
                            "Moving body + fixed wall": "Moving Body + Wall",
                            "Prescribed moving wall": "Moving Wall",
                        }
                    reverse_display_map = {
                        display: value for value, display in display_map.items()
                    }
                    item_texts = [
                        display_map.get(str(item), str(item))
                        for item in combo_items
                    ]
                    display_value = display_map.get(str(val), str(val))
                    if val is not None and display_value not in item_texts:
                        item_texts.append(display_value)
                    widget.addItems(item_texts)
                    if val is not None:
                        idx = widget.findText(display_value)
                        if idx >= 0:
                    ...
                    def _on_combo_changed(
                        ...
                        property_name=name,
                        selected_node=node,
                        value_map=reverse_display_map,
                    ):
                        self.update_property(
                            property_name,
                            value_map.get(value, value),
                        )
                        if property_name == "assembly_connection":
                    ...
                    widget.currentTextChanged.connect(_on_combo_changed)
