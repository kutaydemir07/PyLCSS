...
class WorkbenchBoundaryMixin:
    ...
    def _collect_bc_for_node(self, anchor_node):
        ...
        if anchor_cls == "CrashSolverNode":
            for candidate in self.graph.all_nodes():
                if candidate.__class__.__name__ == "ImpactConditionNode" and (
                    solver_overlay_scope is None
                    or id(candidate) in solver_overlay_scope
                ):
                    scope = (
        ...
        for node in self.graph.all_nodes():
            ...
            if (
                solver_overlay_scope is not None
                and id(node) not in solver_overlay_scope
            ):
                continue
            ...
            # ---- Topology study-definition nodes -------------------------
                ...
                if (
                    ...
                    continue
                joint_type = str(node.get_property("joint_type") or "Joint")
                joint_name = str(node.get_property("joint_name") or "Joint")
                joint_axis = str(node.get_property("axis") or "X")
                anchor_centers = {}
                for port_name, suffix in (("anchor_a", "A"), ("anchor_b", "B")):
                    faces = _get_faces_for_bc_node(node, extra_port_names={port_name})
                    viz = {
                        "constraint_type": f"{joint_type} joint {suffix}",
                        "legend_label": "JOINT INTERFACE",
                        "color": "#FF9F1C",
                        "fixed_dofs": [],
                        "glyph": False,
                    }
                    ...
                    if faces:
                        centers = np.asarray(
                            [self._get_face_centroid(geometry) for geometry in faces],
                            dtype=float,
                        )
                        if centers.ndim == 2 and len(centers):
                            anchor_centers[suffix] = np.mean(centers, axis=0).tolist()
                if "A" in anchor_centers and "B" in anchor_centers:
                    anchors = [anchor_centers["A"], anchor_centers["B"]]
                    midpoint = (
                        0.5
                        * (
                            np.asarray(anchors[0], dtype=float)
                    ...
                    constraint_faces.append(
                        {
                            "pos": midpoint,
                            "viz": {
                                "visual_kind": "joint_link",
                                "constraint_type": f"{joint_type} joint",
                                "legend_label": "JOINT CONNECTION",
                                "color": "#FF9F1C",
                                "fixed_dofs": [],
                                "glyph": False,
            ...
            elif cls == "TopologyNonDesignRegionNode":
                if (
                    anchor_cls.startswith("Topology")
                    and anchor_cls != "TopologyOptVoxelNode"
                    and node is not anchor_node
                ):
                    continue
                regions = _get_faces_for_bc_node(
                    node, extra_port_names={"region_shape"}
                )
                keep_void = (
                    str(node.get_property("region_type") or "").strip().lower()
                    == "keep void"
                )
                viz = {
                    "constraint_type": (
                        "Non-design void" if keep_void else "Non-design material"
                    ),
                    "legend_label": (
                        "NON-DESIGN VOID" if keep_void else "NON-DESIGN MATERIAL"
                    ),
                    "color": "#FF5252" if keep_void else "#69F0AE",
                    "fixed_dofs": [],
                    "glyph": False,
                }
                constraint_faces.extend(
                    {"face": geometry, "viz": viz} for geometry in regions
                )

            elif cls == "TopologyThermalSinkNode":
                if (
                    anchor_cls.startswith("Topology")
                    and anchor_cls != "TopologyOptVoxelNode"
                    and node is not anchor_node
                ...
                faces = _get_faces_for_bc_node(node, extra_port_names={"target_region"})
                viz = {
                    "constraint_type": "Thermal sink",
                    "legend_label": "THERMAL SINK",
                    "color": "#40C4FF",
                    "fixed_dofs": [],
                    "glyph": False,
                }
                ...
                faces = _get_faces_for_bc_node(node, extra_port_names={"target_region"})
                try:
                    total_heat = float(node.get_property("total_heat") or 0.0)
                except Exception:
                    total_heat = 0.0
                for geometry in faces:
                    load_faces.append(
