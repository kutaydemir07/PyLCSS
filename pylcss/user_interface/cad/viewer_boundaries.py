...
class BoundaryOverlayMixin:
    def render_bc_overlays(
        ...
        def _add_impact_wall(item):
            ...
            half_size = max(
                1.5 * _overlay_scale(),
                float(np.linalg.norm(half_extents)) * 0.62,
            )
            corner_0 = center - axis_u * half_size - axis_v * half_size
            corner_1 = center + axis_u * half_size - axis_v * half_size
            corner_2 = center - axis_u * half_size + axis_v * half_size
            ...
            corners = [
                corner_0,
                corner_1,
                center + axis_u * half_size + axis_v * half_size,
                corner_2,
                corner_0,
            ]
            _add_polyline(corners, BC_PALETTE["impact"], line_width=4.0)
            _add_bc_label(center, wall_label, BC_PALETTE["impact"])
        ...
        if load_faces:
            for item in load_faces:
                ...
                if isinstance(item, dict) and item.get("load_type") == "Impact":
                    ...
                    points = _sample_entity_points(entity, max_points=1)
                    if points:
                        _add_bc_label(points[0], "IMPACT", color)
        ...
        if load_vectors:
            ...
            for entry in load_vectors:
                ...
                if isinstance(entry, dict):
                    ...
                    label_position = sample_points[0] if sample_points else centroid
        ...
        if constraint_faces:
            ...
            for item in constraint_faces:
                ...
                label = str(viz.get("legend_label") or "SUPPORT / RESTRAINT")
        ...
        if any(
            ...
            legend_entries.append(("HEAT INPUT", BC_PALETTE["heat"]))
        if any(
            isinstance(item, dict) and item.get("load_type") == "Impact"
            for item in (load_faces or [])
        ):
            legend_entries.append(("IMPACT TARGET / WALL", BC_PALETTE["impact"]))
        if any(
            isinstance(item, dict) and item.get("load_type") == "Selection"
            for item in (load_faces or [])
        ):
            legend_entries.append(("GEOMETRY SELECTION", BC_PALETTE["selection"]))
        if any(
        ...
        if any(
            ...
            legend_entries.append(("IMPACT VELOCITY", BC_PALETTE["velocity"]))
        if any(
            isinstance(entry, dict) and entry.get("quantity_type") == "gravity"
            for entry in (load_vectors or [])
        ):
            legend_entries.append(("GRAVITY / BODY FORCE", BC_PALETTE["gravity"]))
        ...
        if legend_entries:
            ...
            for index, (label, color) in enumerate(legend_entries):
                ...
                actor.SetPosition(0.017, 0.925 - 0.037 * index)
                prop = actor.GetTextProperty()
                prop.SetFontSize(13)
                prop.BoldOn()
