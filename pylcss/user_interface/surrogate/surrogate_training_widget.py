...
from pylcss.surrogate_modeling.active_learning import ActiveLearningConfig
...
class SurrogateTrainingWidget(
    ...
    _FIELD_CHOICES_BY_SOLVER = {
        ...
        "impact": ["stress_vm", "displacement", "von_mises"],
        "crash": ["stress_vm", "displacement", "von_mises"],
        "topopt": ["density"],
    }

    def _update_active_budget_label(self) -> None:
        self.lbl_al_budget.setText(
            str(self.spin_al_rounds.value() * self.spin_al_batch.value())
        )

    def active_learning_values(self) -> dict[str, Any]:
        return {
