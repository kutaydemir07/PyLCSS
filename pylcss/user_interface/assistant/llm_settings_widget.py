...
from pylcss.assistant_systems.config import LLMControlConfig
...
from pylcss.assistant_systems.services.memory import get_secure_storage
...
class LLMSettingsWidget(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        # Local API URL (only shown for local provider)
        self.url_layout_widget = QWidget()
        url_layout = QHBoxLayout(self.url_layout_widget)
        url_layout.setContentsMargins(0, 0, 0, 0)
        url_layout.addWidget(QLabel("Local URL:"))
        
        self.url_input = QLineEdit()
        self.url_input.setPlaceholderText("http://localhost:1234/v1")
        self.url_input.setText(self.config.local_api_url)
        self.url_input.textChanged.connect(self._on_url_changed)
        url_layout.addWidget(self.url_input, 1)
        
        layout.addWidget(self.url_layout_widget)
    ...
    def _load_current_config(self) -> None:
        ...
        try:
            ...
            # Show/hide URL input
            self.url_layout_widget.setVisible(self.config.provider == "local")
        finally:
            # Restore signals
    ...
    def _on_provider_changed(self) -> None:
        ...
        # Show/hide URL input
        self.url_layout_widget.setVisible(provider == "local")
    ...
    def _on_url_changed(self, text: str) -> None:
        ...
        self.config.local_api_url = text.strip()
        self._schedule_save()
    ...
    def refresh_provider(self) -> Optional[LLMProvider]:
        ...
        try:
            kwargs = {}
            if provider_name == "local":
                kwargs["local_api_url"] = self.config.local_api_url
