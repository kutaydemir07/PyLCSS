...
class LLMConfigDialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        self.status_label.setStyleSheet(
            "color: #808080; font-size: 11px; padding: 5px;"
        )
    ...
    def _create_provider_tab(self) -> QWidget:
        ...
        privacy_notice = QLabel(
            "Cloud providers receive the request, recent conversation history "
            "(when enabled), and a snapshot of the active graph, including node "
            "properties. Provider terms and privacy policies apply. Local keeps "
            "requests on the configured local endpoint."
        )
        privacy_notice.setWordWrap(True)
        privacy_notice.setStyleSheet(
            "color: #e7c56b; background: #332d1f; padding: 8px; border-radius: 4px;"
        )
        ...
        self.active_provider_combo.currentIndexChanged.connect(
    ...
    def _create_provider_config_group(self, provider_name: str) -> QGroupBox:
        ...
        show_btn.toggled.connect(
            lambda checked, ki=key_input: ki.setEchoMode(
        ...
        load_btn.clicked.connect(
            lambda checked, pn=provider_name: self._load_models(pn)
        )
    ...
    def _create_generation_tab(self) -> QWidget:
        ...
        self.auto_execute_cb.setToolTip(
            "Execute LLM-suggested actions immediately without asking"
        )
    ...
    def _create_memory_tab(self) -> QWidget:
        ...
        self.memory_cb.setToolTip(
            "Store prompts and responses locally and resend recent messages as context"
        )
        mem_layout.addWidget(self.memory_cb)

        memory_notice = QLabel(
            "Off by default. When enabled, prompts and responses are stored in "
            "plaintext under your per-user PyLCSS data directory. Clear All Memory "
            "deletes the retained conversation file."
        )
        memory_notice.setWordWrap(True)
        memory_notice.setStyleSheet("color: #b8c3d6; font-size: 10px;")
        mem_layout.addWidget(memory_notice)
            ...
            if key_input:
                ...
                if encrypted_key:
                    try:
                        ...
                        logger.warning(
                            "Ignoring an unreadable API key for %s: %s",
                            provider_name,
                            exc,
                        )
                        key_input.clear()
