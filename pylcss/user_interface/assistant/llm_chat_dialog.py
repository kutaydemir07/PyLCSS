...
class LLMChatDialog(QDialog):
    ...
    def _load_provider_from_config(self) -> None:
        ...
        # Load local URL
        self.url_input.setText(llm_config.local_api_url)
        
        # Toggle URL visibility
        self.url_label.setVisible(provider_name == "local")
        self.url_input.setVisible(provider_name == "local")
    ...
    def _on_provider_changed(self) -> None:
        ...
        self.url_label.setVisible(provider_name == "local")
        self.url_input.setVisible(provider_name == "local")
        self.url_input.setText(self._config.llm_control.local_api_url)
    ...
    def _connect_manager_signals(self):
        """Connect to AssistantManager agentic signals."""
        if not self.assistant_manager:
            return
            
        # Agentic system signals
        self.assistant_manager.agentic_result_received.connect(self._on_agentic_result)
        self.assistant_manager.agentic_error_received.connect(self._on_agentic_error)
        self.assistant_manager.agentic_progress.connect(self._on_agentic_progress)
    ...
    def _connect_worker(self, provider_name: str, api_key: str):
        ...
        try:
            ...
            if provider_name == "local":
                kwargs["local_api_url"] = self.url_input.text().strip()
    ...
    def _on_send_clicked(self):
        ...
        # Determine if we should use Agentic Mode via AssistantManager
        use_agentic = False
        if self.assistant_manager and self._config.llm_control.agentic_mode:
            use_agentic = True
        
        if use_agentic:
            logger.info("LLM Dialog: Delegating to Agentic System")
            self.assistant_manager.process_agentic_request(message)
        else:
            # Legacy Single-Shot Mode
            logger.info("LLM Dialog: Using legacy single-shot mode")
    ...
    @Slot(object, str)
    def _on_agentic_result(self, result: dict, original_text: str):
        """Handle result from Agentic system."""
        ...
        self._set_status("Agentic task completed" if success else "Agentic task failed")

    @Slot(str)
    def _on_agentic_error(self, error_msg: str):
        """Handle error from Agentic system."""
        self.thinking_indicator.stop()
        self.send_btn.setEnabled(True)
        self._set_status(f"Error: {error_msg}", error=True)
        self._add_message(f"❌ Error: {error_msg}", is_user=False)

    @Slot(str)
    def _on_agentic_progress(self, message: str):
        """Handle progress update from Agentic system."""
        self._set_status(f"🧠 {message}")
        self.add_agent_thought(message)
