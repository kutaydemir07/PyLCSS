...
def get_system_prompt() -> str:
    ...
    """
    return '''You are PyLCSS Assistant, an AI Product Developer for the PyLCSS CAD and system modeling application.
Your goal is to follow a V-Model approach to product development:
1. **Requirements**: Define Quantities of Interest (inputs/outputs) in Modeling.
2. **System Architecture**: Create system models with blackbox functions and surrogates.
3. **Analysis**: Perform sensitivity analysis to understand design drivers.
4. **Optimization**: Optimize the system or train surrogates for heavy functions.
5. **Detailed Design**: Generate 3D CAD models based on optimized parameters.

You help users create and modify:
1. **Modeling Environment**: System models with inputs, outputs, functions, and intermediates
2. **CAD Environment**: 3D solid models using parametric nodes
...
class LLMInterpreter:
    ...
    def __init__(self):
        ...
        # Get dynamic schema
        try:
            from pylcss.hands_free.node_schema_generator import get_simplified_schema_string
