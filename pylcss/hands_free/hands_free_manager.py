...
from pylcss.hands_free.voice_controller import VoiceController, WHISPER_AVAILABLE
...
from pylcss.hands_free.tts import get_tts, TextToSpeech

# Agentic AI components
try:
    from pylcss.hands_free.agent_orchestrator import AgentOrchestrator
    from pylcss.hands_free.agent_tools import create_pylcss_tools, ToolRegistry
    from pylcss.hands_free.workflow_recorder import WorkflowLibrary, WorkflowRecorder, WorkflowPlayer
...
class HandsFreeManager(QObject):
    ...
    def update_config(self, config: HandsFreeConfig) -> None:
        ...
        logger.info("Updating HandsFreeManager configuration...")
        self.config = config
    ...
    def _get_or_create_llm_overlay(self):
        ...
        if self._llm_overlay is None:
            try:
                from pylcss.hands_free.ui.overlay_widget import OverlayWidget
    ...
    def _process_llm_request(self, text: str) -> None:
        ...
        if self._use_agentic_mode and self._agent_orchestrator:
            self._process_agentic_request(text)
            return
    ...
    def _process_agentic_request(self, text: str) -> None:
    ...
    def _on_agent_step_complete(self, step) -> None:
        ...
        if step.result:
            if step.result.success:
                ...
                msg = f"⚠️ Failed: {step.action.tool_name if step.action else 'unknown'} (Retrying...)"
                logger.warning(f"Agent step failed: {step.result.error}")
