...
"""
Geometry-aware neural-operator backbones.
...
if TORCH_AVAILABLE:
    ...
    class SIRENLayer(nn.Module):
        ...
        def __init__(
            ...
            if in_features < 1 or out_features < 1:
                raise ValueError("SIREN layer dimensions must be positive.")
    ...
    class SIRENNet(nn.Module):
        ...
        def __init__(
            ...
            super().__init__()
            if min(in_dim, hidden_dim, out_dim, n_hidden) < 1:
                raise ValueError("SIREN dimensions and n_hidden must be positive.")
            layers: list[nn.Module] = [
                SIRENLayer(in_dim, hidden_dim, w0=w0_first, is_first=True)
            ]
    ...
    class GeomDeepONet(nn.Module):
        ...
        Inputs
        ------
        params : (batch, n_param)
            Design parameter vector, broadcast across each sample's queries.
        ...
        def __init__(
            ...
            if (
                ...
                raise ValueError(
                    "GeomDeepONet dimensions and layer counts must be positive."
                )
        ...
        def forward(self, params: torch.Tensor, query: torch.Tensor) -> torch.Tensor:
            ...
            if params.dim() != 2 or params.shape[1] != self.n_param:
                raise ValueError(
                    f"params must have shape (batch, {self.n_param}); "
                    f"received {tuple(params.shape)}."
                )
            if query.dim() != 2 or query.shape[1] != 4:
                raise ValueError(
                    "query must have shape (n_query, 4); "
    ...
    class _SpectralConv3d(nn.Module):
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            # x: (B, in_c, R, R, R)
            if x.dim() != 5 or x.shape[1] != self.in_channels:
                raise ValueError(
                    f"x must have shape (batch, {self.in_channels}, R, R, R); "
                    f"received {tuple(x.shape)}."
                )
            if not (x.shape[2] == x.shape[3] == x.shape[4]):
                raise ValueError("Spectral convolution requires a cubic input grid.")
            B, _, R, _, _ = x.shape
    ...
    class GINONet(nn.Module):
        ...
        def __init__(
            ...
            super().__init__()
            if n_param < 0:
                raise ValueError("n_param cannot be negative.")
            if min(out_dim, grid_size, modes, hidden_channels, n_fno_layers) < 1:
                raise ValueError("GINO dimensions and layer counts must be positive.")
            if grid_size < 2:
        ...
        def forward(
            ...
            if sdf.dim() != 5 or sdf.shape[1] != 1:
                raise ValueError(
                    "sdf must have shape (batch, 1, R, R, R); "
                    f"received {tuple(sdf.shape)}."
                )
            if not (sdf.shape[2] == sdf.shape[3] == sdf.shape[4]):
                raise ValueError("sdf must use a cubic background grid.")
            if sdf.shape[2] != self.grid_size:
                raise ValueError(
                    f"sdf grid size {sdf.shape[2]} does not match configured "
                    f"grid_size={self.grid_size}."
                )
            B, _, R, _, _ = sdf.shape
            if params.dim() != 2 or params.shape != (B, self.n_param):
                raise ValueError(
                    f"params must have shape ({B}, {self.n_param}); "
                    f"received {tuple(params.shape)}."
                )
            if query.dim() != 2 or query.shape[1] != 3:
                raise ValueError(
                    "query must have shape (n_query, 3); "
                    f"received {tuple(query.shape)}."
                )
            # Broadcast params as additional input channels.
            if self.n_param > 0:
                p = params.view(B, self.n_param, 1, 1, 1).expand(
                    B, self.n_param, R, R, R
                )
            ...
            for spec, pw in zip(self.spectral, self.pointwise, strict=False):
                x = spec(x) + pw(x)
            ...
            return (
                sampled.squeeze(-1).squeeze(-1).permute(0, 2, 1).contiguous()
            )  # (B, n_query, out_dim)
    ...
    class _TorchRequired:
        def __init__(self, *_args: object, **_kwargs: object) -> None:
            raise RuntimeError("PyTorch is required for neural-operator models.")

    class SIRENLayer(_TorchRequired):  # type: ignore[no-redef]
        """Unavailable placeholder with an actionable error."""

    class SIRENNet(_TorchRequired):  # type: ignore[no-redef]
        """Unavailable placeholder with an actionable error."""

    class GeomDeepONet(_TorchRequired):  # type: ignore[no-redef]
        """Unavailable placeholder with an actionable error."""

    class GINONet(_TorchRequired):  # type: ignore[no-redef]
        """Unavailable placeholder with an actionable error."""
