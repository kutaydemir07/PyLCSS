...
class AssistantManager(QObject):
    ...
    def initialize(self) -> bool:
        ...
        # The dispatcher exposes only in-application graph and workflow tools.
    ...
    def _initialize_llm_provider(self) -> None:
        ...
        if not encrypted_key:
            logger.info("No API key configured for provider %s", provider_name)
            self._llm_provider = None
            return
    ...
    def _initialize_agentic_system(self) -> None:
        ...
        try:
            ...
            # Local OpenAI-compatible servers commonly need no credential.
            # Cloud providers do, so leave the runner dormant until the user
            # supplies one instead of letting a provider constructor read a
            # missing environment variable and emit a startup traceback.
            cloud_provider = "google" if provider == "gemini" else provider
            if cloud_provider != "local" and not api_key:
                logger.info(
                    "Agentic assistant not initialized: no API key for %s",
                    cloud_provider,
                )
    ...
    def process_agentic_request(self, text: str) -> None:
        ...
        if not text:
            ...
            return
        logger.info(
            "Assistant request started (provider=%s, characters=%d)",
            self.config.llm_control.provider,
            len(text),
        )
