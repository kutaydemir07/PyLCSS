...
from pylcss.assistant_systems.tools.registry import Tool, ToolRegistry, get_cad_schema_for_prompt
...
2. **Use Core Primitives**: Maps ambiguous terms like "shell", "peel", or "hollow" to standard operations.
   - Example (Drilling): Create Box -> Create Cylinder -> Boolean (operation='Cut').
   - Example (Gearbox): Create Case (Box) -> Create Gears (Boxes/Cylinders) -> Position and connect.
3. **STRICT Tool & Node Usage**: 
   - **NEVER** invent tools. Use ONLY the tool names listed.
   - **NEVER** invent CAD node types. Use ONLY the types in the "Available CAD Node Types" list.
   - **NO** `com.cad.spur_gear`, **NO** `com.cad.motor`, **NO** `com.helical_gear`, **NO** `create_helical_gear`. These DO NOT EXIST.
   - If a specific object isn't in the list, you **MUST** build it using primitives (box, cylinder, etc.) and booleans.
4. **Break Down**: Split complex tasks into atomic `create_cad_geometry` and `connect_cad_nodes` calls.
5. **Connection Syntax**: 
   - Schema: `{{"from": "node_id.port", "to": "node_id.port"}}`.
   - Typical ports: `shape`, `shape_a`, `shape_b`.
...
{{
  ...
  "reasoning": "Step-by-step logic. Explain how you mapped complex objects to primitives.",
  "steps": [
...
- **Tool Used**: {tool_name}
- **Output**: {output}
- **Observation (Graph State)**: {observation}

## Expected Outcome for this Step
{expected}

## Original User Request (Overall Goal)
{user_request}

## Evaluation Guidelines
1. **Intermediate Steps (Step < Total)**: 
   - **DO NOT** reject just because the final goal isn't reached yet.
   - **APPROVE** if the tool executed successfully and the observation matches the **Expected Outcome** for this specific step.
   - Example: If the step is `switch_tab`, and the result is `success`, approve it.
2. **Final Step (Step == Total)**:
   - Evaluated against the **Original User Request**.
3. **Reject** ONLY if:
   - The tool failed (`success`: False).
   - The specific expected outcome for this step was not achieved.
   - The step introduces an error that makes the overall goal impossible.
...
{{
  ...
  "design_feedback": "Short comment on intent",
  "severity": 1-5
}}
```
    ...
    def _parse_plan(self, response: str) -> ExecutionPlan:
        ...
        if data:
            try:
                ...
                # Sanitize response for logging to prevent UnicodeEncodeError on some terminals
                safe_response = response.encode('ascii', errors='replace').decode('ascii')
                logger.error(f"Raw Response: {safe_response}")
...
class CriticAgent(BaseAgent):
    ...
    def evaluate(
        ...
        prompt = CRITIC_PROMPT.format(
            ...
            observation=current_state[:1000] if current_state else "{}",
