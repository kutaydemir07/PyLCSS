...
COMPONENT_LICENSES: Dict[str, Dict[str, str]] = {
    "ccx": {
        "license": "GNU General Public License version 2 (upstream notices allow version 2 or later)",
        "license_url": "https://www.calculix.de/",
        "source_url": "https://www.dhondt.de/ccx_2.23.src.tar.bz2",
    },
    "radioss": {
        "license": "GNU Affero General Public License version 3 or later",
        "license_url": (
            "https://github.com/OpenRadioss/OpenRadioss/blob/"
            "latest-20260615/LICENSE.md"
        ),
        "source_url": (
            "https://github.com/OpenRadioss/OpenRadioss/tree/latest-20260615"
        ),
    },
    "freecad": {
        "license": "GNU Lesser General Public License version 2.1",
        "license_url": "https://github.com/FreeCAD/FreeCAD/blob/1.1.3/LICENSE",
        "source_url": "https://github.com/FreeCAD/FreeCAD/tree/1.1.3",
    },
}
...
def _print_component_license(key: str, asset: SolverAsset) -> None:
    legal = COMPONENT_LICENSES[key]
    print(f"[{asset.name}] third-party software; not covered by the PyLCSS license.")
    print(f"  License: {legal['license']}")
    print(f"  Terms:   {legal['license_url']}")
    print(f"  Source:  {legal['source_url']}")


def _write_component_notice(key: str, asset: SolverAsset, install_dir: Path) -> None:
    legal = COMPONENT_LICENSES[key]
    install_dir.mkdir(parents=True, exist_ok=True)
    notice = "\n".join(
        (
            f"{asset.name} - third-party component",
            "",
            "This component is not covered by the PyLCSS license.",
            f"License: {legal['license']}",
            f"License terms: {legal['license_url']}",
            f"Corresponding source: {legal['source_url']}",
            f"Downloaded artifact: {asset.url}",
            "",
            "Preserve this notice and all notices included by the upstream",
            "publisher if you redistribute the component.",
            "",
        )
    )
    (install_dir / "PYLCSS-THIRD-PARTY-NOTICE.txt").write_text(
...
def _remove_excluded_solver_artifacts(
    key: str,
    extract_dir: Path,
    archive_path: Path,
) -> None:
    """Remove unused components that PyLCSS intentionally does not distribute."""
    if key != "ccx":
        return

    for candidate in extract_dir.rglob("*"):
        if candidate.is_file() and candidate.name.lower() == "tetgen.exe":
            candidate.unlink()
            print(f"  removed unused third-party executable: {candidate.name}")

    # The upstream archive contains the excluded executable, so do not retain
    # the download cache after extracting the CalculiX solver used by PyLCSS.
...
def install_solver(key: str, force: bool, url_override: Optional[str]) -> Dict[str, str]:
    ...
    _print_component_license(key, asset)
    ...
    _write_component_notice(key, asset, install_dir)
    ...
    _remove_excluded_solver_artifacts(key, extract_dir, archive_path)
